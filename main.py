...
with open(f"./compliance/reports/report-{current.strftime('%d-%m-%Y_%H:%M:%S')}.txt", "w") as file:
	file.write(f"**********  Compliance Report, Ran {current.strftime('%d-%m-%Y %H:%M:%S')}  **********\n\n")
	for device in my_devices:
		...
		for policy in my_policies:
			if policy.get("platform") == device.get("NOS") and 'router' in policy.get('device_types'):
				...
				result = parse_policy(configPair, f"./compliance/currentConfigs/{device.get('deviceName')}.txt")
				if not result:
					file.write(f"- Violation {viol_id} - {policy.get('name')} is not applied\n")
					viol_id += 1
		if viol_id == 1:
