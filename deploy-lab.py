...
for node_id, device in enumerate(my_devices, 1):
    filepath = os.path.exists(
        f"./renderedTemplates/deployment/{device.get('deviceName')}.txt"
    )
    if not filepath:
    ...
    if device.get("NOS") == "EOS":
        dev_object = Switch(
            device.get("deviceName"), left=base_left, top=base_top
        ).to_json()
    ...
    time.sleep(0.1)
    with open(
        f"./renderedTemplates/deployment/{device.get('deviceName')}.txt", "r"
    ) as configfile:
...
for x, device in enumerate(mgmt_info, 1):
    NOS = [
        ...
        if a_device.get("deviceName") == mgmt_info[x - 1][0]
    ]
    if NOS[0] == "IOS":
