...
def load_api_data():
    ...
    for result in r["results"]:
        ...
        my_devices.append({"Vendor": vendor, "Model": model, "deviceName": device, "Management IP": mgmt_ip, "Role": role, "Region": region, "Site": site, "TACACS Server": tacacs})
    ...
    for device in my_devices:
        for x, current_device in enumerate(current_data):
            for key, value in current_device.items():
                if current_device[key] == 'null' and device[key] == None:
                    pass
                elif current_device["deviceName"] == device["deviceName"] and current_device[key] != device[key]:
